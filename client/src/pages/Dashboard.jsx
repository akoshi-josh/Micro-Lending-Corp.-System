...
export default function Dashboard() {
  ...
  return (
    <div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
        <div className="px-5 py-4 border-b border-gray-200">
          <span className="text-base font-bold text-gray-800">Upcoming Payments</span>
        </div>
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50">
          ...
          <tbody>
            ...
            {data?.upcoming_payments?.map((u, i) => (
              <tr key={i} className="border-t border-gray-100 hover:bg-gray-50 transition-colors">
                <td className="px-5 py-3 text-sm font-semibold text-gray-800">{u.full_name}</td>
                <td className="px-5 py-3 text-sm text-gray-600">{formatDate(u.due_date)}</td>
                <td className="px-5 py-3 text-sm text-gray-500 capitalize">{u.payment_frequency?.replace('_', '-')}</td>
                <td className="px-5 py-3 text-right text-sm font-bold text-gray-800">{formatCurrency(u.amount_due)}</td>
